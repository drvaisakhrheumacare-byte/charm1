...
# --- CONFIGURATION ---
# Default URL (can be changed in the web UI)
...
def get_sheet_csv_url(original_url):
    ...
    try:
        ...
        # 2. Get the GID (The specific tab)
        # Defaults to 0 (first tab) if not found
        gid = "0" 
...
def create_coupon_content(cell, name, emp_id, code, date_label):
    ...
    run1.bold = True
    run1.font.size = Pt(9)
    run1.font.color.rgb = RGBColor(0, 100, 0) # Dark Green
...
def generate_docx(df, selected_date, prefix):
    ...
    # Looks for 'name' and 'id'/'code' in headers to avoid errors
    ...
    for index, row in df.iterrows():
        ...
        # Create Table (13 rows x 5 cols)
        table = doc.add_table(rows=13, cols=5)
        table.style = 'Table Grid'
        
        # Set Row Height
        for row_obj in table.rows:
            row_obj.height = Cm(2.1) 
...
def check_password():
    ...
    if "password" not in st.secrets: return True # Bypass if local/no secret set
...
if check_password():
    ...
    st.write("Live connection to Employee Google Sheet.")
    ...
    with st.container():
        ...
        with col1:
            default_date = datetime.datetime.now().strftime("%B %Y")
            selected_date = st.text_input("Month & Year", value=default_date)
        with col2:
